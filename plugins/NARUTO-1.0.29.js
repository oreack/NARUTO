  ...
  smd(
    ...
    async ({ reply, chat, isGroup, sender, error }) => {
      try {
        ...
        const daily = await eco.daily(sender, "rxcoder", 500); //give 500 for daily, can be changed
        if (daily.cd) {
  ...
  smd(
    ...
    async (message) => {
      try {
        ...
        const balance = await eco.balance(users, "rxcoder");
        await eco.deduct(users, "rxcoder", balance.wallet);
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const balance = await eco.balance(user, "rxcoder");
        switch (value) {
          case "1000":
          case "1":
            if (k > balance.wallet)
            ...
            const deduct1 = await eco.deduct(user, "rxcoder", 100);
            const add1 = eco.giveCapacity(user, "rxcoder", 1000);
            return await message.reply(
          ...
          case "3":
            ...
            const deduct3 = await eco.deduct(user, "rxcoder", 10000);
            const add3 = eco.giveCapacity(user, "rxcoder", 10000000);
            return await message.reply(
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const deposit = await eco.deposit(message.sender, "rxcoder", d);
        const balance = await eco.balance(message.sender, "rxcoder");
        if (deposit.noten)
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const balance = await eco.balance(user1, "rxcoder");
        let a = balance.wallet < parseInt(word);
        //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        if (a == true)
          return message.reply("you dont have sufficient money to transfer👎");

        const deduct = await eco.deduct(user1, "rxcoder", value[0]);
        const give = await eco.give(user2, "rxcoder", value[0]);
        return await message.reply(
  ...
  smd(
    ...
    async (message) => {
      try {
        ...
        if (mongoschemas == "false")
          return message.reply("*🚦Economy* is not active in current group.");
        const balance = await eco.balance(message.sender, "rxcoder"); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        return await message.reply(
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        await eco.give(users, "rxcoder", parseInt(match.split(" ")[0]));
  ...
  smd(
    ...
    async (message) => {
      try {
        ...
        const user2 = users;
        const k = 1000;
        const balance1 = await eco.balance(user1, "rxcoder");
        const balance2 = await eco.balance(user2, "rxcoder");
        ...
        switch (random) {
          ...
          case "rob":
            ...
            await eco.deduct(user2, "rxcoder", deduff);
            await eco.give(message.sender, "rxcoder", deduff);
            await message.reply(
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        const withdraw = await eco.withdraw(user, "rxcoder", query);
        if (withdraw.noten)
  ...
  smd(
    ...
    async (message, match) => {
      try {
        ...
        message.reply(
          media,
          { packname: "rxcoder", author: "Economy" },
          "sticker"
        );
        const f = [
          "up",
        ...
        if (r == opp) {
          let give = await eco.give(user, "rxcoder", twice); //message.react('⭐️')
          ...
          let deduct = await eco.deduct(user, "rxcoder", texts[0]);
    ...
    async (message, match) => {
      try {
        ...
        if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0) {
          ...
          const user = message.sender;
          const k = 100;
          const balance1 = await eco.balance(user, "rxcoder");
          if (k > balance1.wallet)
            return message.reply(
          ...
          if (match.split(" ")[0]) {
            ...
            if (value <= balance.wallet) {
              ...
              if (f1 !== f2 && f2 !== f3) {
                const deduct1 = await eco.deduct(user, "rxcoder", deduff);
                return message.reply(
                  `${mess1}\n\n*Big Lose -->* _🪙${deduff}_`
                );
              } else if (f1 == f2 && f2 == f3) {
                const give1 = await eco.give(user, "rxcoder", deduff / 2);
                return message.reply(
                  `${mess2}\n*_Little Jackpot -->* _🪙${deduff / 2}_`
                );
              } else if (f1 == f2 && f2 !== f3) {
                const give2 = await eco.give(user, "rxcoder", deduff);
                return message.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`);
              } else if (f1 !== f2 && f1 == f3) {
                const deduct2 = await eco.deduct(user, "rxcoder", deduff);
                return message.reply(
                  `${mess5}\n\n*Small Lose -->* _🪙${deduff}_`
                );
              } else if (f1 !== f2 && f2 == f3) {
                const give4 = eco.give(user, "rxcoder", deduff);
                return message.reply(
                  `${mess3}\n\n*Small Win -->* _🪙${deduff}_`
                );
              } else if (f1 == f2 && f2 == f3 && f3 == f4) {
                const give5 = eco.give(user, "rxcoder", deduff * 20);
                return message.reply(
          ...
          if (f1 !== f2 && f2 !== f3) {
            const deduct1 = await eco.deduct(user, "rxcoder", 50);
            message.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`);
          } else if (f1 == f2 && f2 == f3) {
            const give1 = await eco.give(user, "rxcoder", 100);
            message.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`);
          } else if (f1 == f2 && f2 !== f3) {
            const give2 = await eco.give(user, "rxcoder", 20);
            message.reply(`${mess3}\n*Small Win -->* _🪙20_`);
          } else if (f1 !== f2 && f1 == f3) {
            const deduct2 = await eco.deduct(user, "rxcoder", 20);
            message.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`);
          } else if (f1 !== f2 && f2 == f3) {
            const give4 = eco.give(user, "rxcoder", 20);
            message.reply(`${mess3}\n\n*Small Win -->* _🪙20_`);
          } else if (f1 == f2 && f2 == f3 && f3 == f4) {
            const give5 = eco.give(user, "rxcoder", 1000);
            message.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`);
          } else {
    ...
    async (message) => {
      try {
        ...
        if (mongoschemas == "false")
          return message.reply("*🚦Economy* is not active in current group.");
        const kg = 100;
        const balance1 = await eco.balance(message.sender, "rxcoder");
        ...
        if (lol) {
          ...
          const give2 = await eco.give(message.sender, "rxcoder", deduff * 2);
          ...
          const deduct1 = await eco.deduct(message.sender, "rxcoder", deduff);
          let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `;
          let str = st
            .replace(/1/g, `🔴`)
            .replace(/2/g, `🔵`)
